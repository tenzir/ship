...
When you are ready to ship the stable release, rerun `tenzir-ship release create` without `--rc` and the latest outstanding release candidate becomes the matching stable release automatically. To ship a specific candidate exactly, use `tenzir-ship release create 1.2.3 --from v1.2.3-rc.2`. If you want to ignore existing release candidates and cut the stable release from the current unreleased queue instead, use `--current-unreleased`.
